...
import { QuerySelect, Term } from '@traqula/rules-sparql-1-1';
...
export async function processQueries(metadataPath: string, queriesPath: string, outputPath: string): Promise<void> {
  const metadataRaw = fs.readFileSync(metadataPath, 'utf-8');
  const metadata: MetadataEntry[] = JSON.parse(metadataRaw).sequenceElements;
  ...
  for (let i = 0; i < Math.min(metadata.length, queries.length); i++) {
    ...
    const ast: QuerySelect = <QuerySelect>parser.parse(queryString);

    const astPrefixes: Record<string, string> = {};
    ...
    const pathPatterns: Algebra.Path[] = [];
    algebraUtils.visitOperation(ast, {
    ...
    const scanToPattern: Map<JoinTreeNode, Algebra.Pattern | Algebra.Path> = new Map();
...
function reorderScope(
  patterns: any[], 
  planRoot: JoinTreeNode, 
  scanToPattern: Map<JoinTreeNode, Algebra.Pattern | Algebra.Path>
): any[] {
  const elements = new Set<any>();
  const nonTripleElements: { node: any, idx: number }[] = [];
  const elemToTriples = new Map<any, Set<any>>();
  ...
  patterns.forEach((p, i) => {
    ...
    if (p.subType === 'bgp') {
      ...
      if (p.patterns && (p.subType === 'optional' || p.subType === 'group')) {
        ...
        p.patterns = p.patterns.map((branch: any) => {
          if (branch.patterns) {
  ...
  function buildJoinTree(node: JoinTreeNode): any {
    ...
    const allItems = [];
    if (currentMatch) allItems.push(currentMatch);
    ...
    const parts = [];
    if (triples.length > 0) parts.push(createBgpPattern(triples));
  ...
  const unmappedRaw = Array.from(elements).filter((e: any) => isTriple(e));
  const unmappedComplex = Array.from(elements).filter((e: any) => !isTriple(e));
  
  if (unmappedRaw.length > 0) branches.push(createBgpPattern(unmappedRaw));
  branches.push(...unmappedComplex);

  function getMinIndex(node: any): number {
    if (!node) return Infinity;
    if (originalIndexMap.has(node)) return originalIndexMap.get(node)!;
    
    let min = Infinity;
    if (node.triples) node.triples.forEach((t: any) => { min = Math.min(min, getMinIndex(t)); });
    if (node.patterns) node.patterns.forEach((p: any) => { min = Math.min(min, getMinIndex(p)); });
    return min;
  }
...
function isTriple(e: any): boolean {
...
function collectAllTriples(patterns: any[]): any[] {
  const triples: any[] = [];
  for (const p of patterns) {
    ...
    if (isTriple(p)) {
      ...
      if (p.triples) triples.push(...p.triples);
      else triples.push(p); 
    } else if (p.patterns) {
...
function subtreeContainsTriples(node: JoinTreeNode, triples: Set<any>, scanToPattern: Map<JoinTreeNode, any>): boolean {
  let found = false;
  function check(n: JoinTreeNode) {
    if (found) return;
...
export function createBgpPattern(triples: any[]): any {
  return { type: 'pattern', subType: 'bgp', triples, loc: DUMMY_LOC };
}

export function createGroupPattern(patterns: any[]): any {
  return { type: 'pattern', subType: 'group', patterns, loc: DUMMY_LOC };
}

export function createUnionPattern(patterns: any[]): any {
  return { type: 'pattern', subType: 'union', patterns, loc: DUMMY_LOC };
}

// --- Triple & Path Matching Logic ---

function matchTriplePatterns(
  scanToPattern: Map<JoinTreeNode, Algebra.Pattern | Algebra.Path>,
  joinPlan: JoinTreeNode,
  quadPatterns: Algebra.Pattern[],
  pathPatterns: Algebra.Path[],
  prefixes: Record<string, string>,
) {
  joinPlan.children.forEach(child =>
    matchTriplePatterns(scanToPattern, child, quadPatterns, pathPatterns, prefixes)
  );
  ...
  if (matchScan) {
    ...
    let found: any = quadPatterns.find(p => isPatternEqual(p, algebraPattern, prefixes));
    
    if (!found) {
      found = pathPatterns.find(p => isTermEqual(<any>p.subject, algebraPattern.subject, prefixes) &&
                                     isTermEqual(<any>p.object, algebraPattern.object, prefixes));
    }
    
    if (found) scanToPattern.set(joinPlan, found);
    return;
  }
...
function getPathIri(predicate: any): string | null {
  if (!predicate) return null;
  if (predicate.value) return predicate.value; 
  if (predicate.iri && predicate.iri.value) return predicate.iri.value; 
  if (predicate.items && predicate.items.length > 0) return getPathIri(predicate.items[0]); 
  if (predicate.path) return getPathIri(predicate.path); 
  return null;
}

function isEqualPathPattern(pathPattern: any, qLeverPattern: any, prefixes: Record<string, string>): boolean {
...
function parseTriple(tripleString: string): any {
  const dummyQuery = `SELECT * WHERE { ${tripleString} . }`;
  try {
    const ast: any = parser.parse(dummyQuery);
    const wherePatterns = ast.where?.patterns || ast.where || [];
    const bgpNode = wherePatterns.find((p: any) => p.type === 'pattern' && p.subType === 'bgp');
    if (bgpNode?.triples?.length > 0) return bgpNode.triples[0];
...
function isPatternEqual(p1: any, p2: any, prefixes: Record<string, string>): boolean {
...
function getExpandedValue(term: any, prefixes: Record<string, string>): string {
...
function isTermEqual(t1: Term, t2: Term, prefixes: Record<string, string>) {
